...
export const fullHistory = async (req, res, next) => {
  try {
    ...
    const [
      ...
      Prescription.find({ patient: id }).populate('doctor', 'user').sort({ createdAt: -1 }).limit(50),
      LabOrder.find({ patient: id }).populate('doctor', 'user').sort({ createdAt: -1 }).limit(50),
      Billing.find({ patient: id }).populate('doctor', 'user').sort({ createdAt: -1 }).limit(50),
      Appointment.find({ patient: id }).populate('doctor', 'user').sort({ date: -1 }).limit(50),
      IPDAdmission.find({ patient: id }).populate('ward', 'name').populate('bed', 'bedNo').populate('admittingDoctor', 'user').populate('dischargingDoctor', 'user').sort({ admissionDate: -1 }).limit(50),
      QueueToken.find({ patient: id }).populate('doctor', 'user').sort({ createdAt: -1 }).limit(50),
